...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'social_book.settings')  # Replace with your actual Django settings module path
django.setup()

from django.contrib.auth import get_user_model  # For fetching CustomUser model
...
DATABASE_URL = "postgresql://myuser:password@localhost:5432/social_book"  # Use the correct DB credentials
...
def transfer_users_to_social_book():
    ...
    for user in users:
        new_user = User(
            ...
            password=user.password  # Handle the password carefully
        )
